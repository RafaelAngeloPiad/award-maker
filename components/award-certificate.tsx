...
import { Award, Medal, Ribbon, Star, Trash2, Edit2, Image as ImageIcon, Type, Award as AwardIcon, UserSquare, Palette, Download, Settings2, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function AwardCertificate() {
	...
	const [ornamentalCornersColor, setOrnamentalCornersColor] = useState('#d4af37'); // New state variable
	const [decorativeElementsColor, setDecorativeElementsColor] = useState('#8b7355'); // New state variable
	...
	const [certificateStyle, setCertificateStyle] = useState<'default' | 'wide'>('default');
	const [presentationText, setPresentationText] = useState('This certificate is proudly presented to:');
			...
			reader.onload = (e) => {
				try {
					...
					setImportAlert({
						...
						type: 'error',
						title: 'Import Failed',
						message: `Failed to import: ${err instanceof Error ? err.message : 'Invalid format'}`
					});
					setTimeout(() => {
						setImportAlert((prev) => ({ ...prev, show: false }));
					}, 3000);
			...
			<div className="flex flex-col 2xl:flex-row gap-8 max-w-[1800px] mx-auto">
				...
				<div className="w-full 2xl:w-[600px] 2xl:flex-shrink-0">
					<div className="flex items-center gap-2 mb-6">
						...
						<h1 className="text-3xl font-bold text-gray-800">Certificate Settings</h1>
					</div>

					<Tabs defaultValue="general" className="w-full">
						<TabsList className="w-full grid grid-cols-7 gap-2 mb-20">
							<TabsTrigger value="general" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<Settings2 className="w-4 h-4" />
									<span className="text-sm font-medium">General</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="images" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<ImageIcon className="w-4 h-4" />
									<span className="text-sm font-medium">Images</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="colors" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<Type className="w-4 h-4" />
									<span className="text-sm font-medium">Colors</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="awards" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<AwardIcon className="w-4 h-4" />
									<span className="text-sm font-medium">Awards</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="signatories" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<UserSquare className="w-4 h-4" />
									<span className="text-sm font-medium">Signatories</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="styles" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
									<Palette className="w-4 h-4" />
									<span className="text-sm font-medium">Styles</span>
								</div>
							</TabsTrigger>
							<TabsTrigger value="export" className="bg-white data-[state=active]:bg-gray-100 rounded-lg hover:bg-gray-50 transition-all shadow-sm">
								<div className="flex flex-col items-center gap-2 py-3 px-2">
						...
						{importAlert.show && (
							<div className="fixed top-4 right-4 z-50 animate-in slide-in-from-top-2">
								<Alert className={`w-96 ${importAlert.type === 'success' ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
									{importAlert.type === 'success' ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <AlertCircle className="h-4 w-4 text-red-600" />}
									<AlertTitle className={`${importAlert.type === 'success' ? 'text-green-800' : 'text-red-800'}`}>{importAlert.title}</AlertTitle>
									<AlertDescription className={`${importAlert.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>{importAlert.message}</AlertDescription>
								</Alert>
							</div>
						)}
						...
						<div className="bg-white rounded-lg shadow-sm p-6">
								...
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											...
											<p className="text-sm text-gray-500">Configure global certificate settings</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Certificate Details</h3>
												<div className="space-y-4">
													<div>
														<Label htmlFor="mainTitle" className="text-sm">
															Main Title
														</Label>
														<Input id="mainTitle" type="text" value={mainTitle} onChange={(e) => setMainTitle(e.target.value)} placeholder="Enter certificate title" className="mt-1" />
													</div>
													<div>
														<Label htmlFor="presentationText" className="text-sm">
															Presentation Text
														</Label>
														<Textarea id="presentationText" value={presentationText} onChange={(e) => setPresentationText(e.target.value)} placeholder="Enter presentation text" className="mt-1" />
													</div>
													<div className="flex items-center p-3 bg-white rounded-lg border border-gray-200">
														...
														<Label htmlFor="borderIcons" className="ml-3">
															<div className="space-y-1">
																<p className="text-sm font-medium">Border Icons</p>
																<p className="text-xs text-gray-500">Toggle the Border icons</p>
															</div>
														</Label>
													</div>
												</div>
										...
										<div className="space-y-4">
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												...
												<ul className="text-sm text-blue-700 space-y-1">
													<li>• The main title appears at the top of all certificates</li>
													<li>• Presentation text appears above recipient&apos;s name</li>
								...
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											...
											<p className="text-sm text-gray-500">Manage certificate images and opacity</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												...
												<div className="grid grid-cols-1 gap-4">
													...
													<FileUpload onFileSelect={handleBackgroundUpload} label="Background" />
													<label className="text-base">For Corner Images : </label>
													<label className="text-sm">Top Left : </label>
													<FileUpload onFileSelect={handleTopLeftUpload} label="Top Left" />
													<label className="text-sm">Top Right : </label>
													<FileUpload onFileSelect={handleTopRightUpload} label="Top Right" />
													<label className="text-sm">Bottom Left : </label>
													<FileUpload onFileSelect={handleBottomLeftUpload} label="Bottom Left" />
													<label className="text-sm">Bottom Right : </label>
													<FileUpload onFileSelect={handleBottomRightUpload} label="Bottom Right" />
												</div>
											</div>
										</div>
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Opacity Settings</h3>
												<div className="space-y-6">
													<div className="space-y-2">
														<div className="flex justify-between">
															<Label htmlFor="backgroundOpacity" className="text-sm">
																Background Opacity
															</Label>
															<span className="text-sm text-gray-500">{(backgroundOpacity * 100).toFixed(0)}%</span>
														</div>
														<Input id="backgroundOpacity" type="range" min="0" max="1" step="0.1" value={backgroundOpacity} onChange={(e) => setBackgroundOpacity(parseFloat(e.target.value))} className="flex-1" />
													</div>

													<div className="space-y-2">
														<div className="flex justify-between">
															<Label htmlFor="cornerOpacity" className="text-sm">
																Corner Images Opacity
															</Label>
															<span className="text-sm text-gray-500">{(cornerOpacity * 100).toFixed(0)}%</span>
														</div>
														<Input id="cornerOpacity" type="range" min="0" max="1" step="0.1" value={cornerOpacity} onChange={(e) => setCornerOpacity(parseFloat(e.target.value))} className="flex-1" />
													</div>
												</div>
											</div>
											...
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">Tips</h3>
												<ul className="text-sm text-blue-700 space-y-1">
													<li>• Adjust background opacity for better text readability</li>
													<li>• Corner images work best at lower opacity levels</li>
													<li>• Changes apply to all certificates</li>
												</ul>
											</div>
										</div>
							...
							<TabsContent value="colors" className="mt-0">
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											<h2 className="text-xl font-semibold text-gray-800">Color Settings</h2>
											<p className="text-sm text-gray-500">Customize Element colors and styles</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
													...
													{[
														...
														<div key={item.id} className="space-y-2">
															...
															<div className="flex gap-2">
																<Input id={item.id} type="color" value={item.value} onChange={(e) => item.setter(e.target.value)} className="w-12 h-8" />
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Decorative Elements</h3>
												<div className="space-y-4">
													{[
														{
											...
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												...
												<ul className="text-sm text-blue-700 space-y-1">
													...
													<li>• Consider the background color when choosing font colors</li>
													<li>• Changes apply to all certificates</li>
												</ul>
											</div>
										</div>
							...
							<TabsContent value="awards" className="mt-0">
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											<h2 className="text-xl font-semibold text-gray-800">Award Management</h2>
											<p className="text-sm text-gray-500">Create and edit award certificates</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Award Details</h3>
												<div className="space-y-4">
													<div>
													...
													{editingAwardIndex !== null ? (
														<Button onClick={() => updateAward(editingAwardIndex)} className="w-full bg-blue-600 hover:bg-blue-700">
															Update Award
														</Button>
													) : (
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Current Awards</h3>
												<div className="space-y-3">
													{awards.map((award, index) => (
														<div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
															<div className="space-y-1">
																<p className="font-medium text-sm">{award.recipient}</p>
															...
															<div className="flex gap-2">
																...
																<Button onClick={() => removeAward(index)} size="sm" variant="outline" className="h-8 w-8 text-red-600 hover:text-red-700">
																	<Trash2 className="h-4 w-4" />
																</Button>
															</div>
														</div>
													))}
													{awards.length === 0 && <p className="text-sm text-gray-500 text-center py-4">No awards added yet</p>}
												</div>
											</div>
											...
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">Tips</h3>
												<ul className="text-sm text-blue-700 space-y-1">
													<li>• Each award will generate a separate certificate</li>
							...
							<TabsContent value="signatories" className="mt-0">
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											<h2 className="text-xl font-semibold text-gray-800">Signatory Management</h2>
											<p className="text-sm text-gray-500">Manage certificate signatories</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Add Signatory</h3>
												<div className="space-y-4">
													<div>
													...
													{editingSignatoryIndex !== null ? (
														<Button onClick={() => updateSignatory(editingSignatoryIndex)} className="w-full bg-blue-600 hover:bg-blue-700">
															Update Signatory
														</Button>
													) : (
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Current Signatories</h3>
												<div className="space-y-3">
													{signatories.map((signatory, index) => (
														<div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-200">
															...
															<div className="flex gap-2">
																...
																<Button onClick={() => removeSignatory(index)} size="sm" variant="outline" className="h-8 w-8 text-red-600 hover:text-red-700">
																	<Trash2 className="h-4 w-4" />
																</Button>
															</div>
														</div>
													))}
													{signatories.length === 0 && <p className="text-sm text-gray-500 text-center py-4">No signatories added yet</p>}
												</div>
											</div>
											...
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">Tips</h3>
												<ul className="text-sm text-blue-700 space-y-1">
													<li>• Add multiple signatories for official certificates</li>
							...
							<TabsContent value="styles" className="mt-0">
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											<h2 className="text-xl font-semibold text-gray-800">Style Settings</h2>
											<p className="text-sm text-gray-500">Configure certificate layout</p>
										</div>
									...
									<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												<h3 className="text-sm font-medium text-gray-700 mb-4">Layout Options</h3>
												<div className="space-y-4">
													<div className="flex items-center p-3 bg-white rounded-lg border border-gray-200">
														<input type="radio" id="defaultStyle" name="certificateStyle" checked={certificateStyle === 'default'} onChange={() => setCertificateStyle('default')} className="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500" />
														<Label htmlFor="defaultStyle" className="ml-3">
															<div className="space-y-1">
																<p className="text-sm font-medium">Default Style (Portrait)</p>
																<p className="text-xs text-gray-500">Best for short certificates</p>
															</div>
														</Label>
													</div>
													<div className="flex items-center p-3 bg-white rounded-lg border border-gray-200">
														<input type="radio" id="wideStyle" name="certificateStyle" checked={certificateStyle === 'wide'} onChange={() => setCertificateStyle('wide')} className="h-4 w-4 border-gray-300 text-blue-600 focus:ring-blue-500" />
														<Label htmlFor="wideStyle" className="ml-3">
															<div className="space-y-1">
																<p className="text-sm font-medium">Wide Style (Landscape)</p>
																<p className="text-xs text-gray-500">Best for long certificates</p>
															</div>
														</Label>
													</div>
												</div>
										...
										<div className="space-y-4">
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">Layout Tips</h3>
												<ul className="text-sm text-blue-700 space-y-1">
													<li>• Portrait style fits two certificates per page</li>
													<li>• Landscape style uses one certificate per page</li>
							...
							<TabsContent value="export" className="mt-0">
								<div className="space-y-6">
									<div className="flex items-center justify-between border-b pb-4">
										<div className="space-y-1">
											<h2 className="text-xl font-semibold text-gray-800">Import/Export Options</h2>
											<p className="text-sm text-gray-500">Import/Export data and certificates</p>
										</div>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
										...
										<div className="space-y-4">
												...
												<div className="space-y-4">
													<div>
														<div className="flex justify-between items-center mb-2">
															...
															<Button onClick={downloadAwardsTemplate} variant="outline" size="sm" className="text-xs">
																Download Template
															</Button>
														</div>
														<Input id="awardsImport" type="file" accept=".csv" onChange={handleAwardsCSVImport} className="mt-1" />
													</div>
													<div>
														<div className="flex justify-between items-center mb-2">
															...
															<Button onClick={downloadSignatoriesTemplate} variant="outline" size="sm" className="text-xs">
																Download Template
															</Button>
														</div>
														<Input id="signatoriesImport" type="file" accept=".csv" onChange={handleSignatoriesCSVImport} className="mt-1" />
													</div>
												</div>
											</div>
											...
											{/* Enhanced Tips Card */}
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">CSV Format Guide</h3>
												<div className="space-y-3">
													<div>
														<p className="text-sm font-medium text-blue-800">Awards CSV Format:</p>
														<code className="text-xs text-blue-700 block mt-1 bg-blue-50/50 p-2 rounded">recipient,title,description</code>
													</div>
													<div>
														<p className="text-sm font-medium text-blue-800">Signatories CSV Format:</p>
														<code className="text-xs text-blue-700 block mt-1 bg-blue-50/50 p-2 rounded">name,title</code>
													</div>
										...
										<div className="space-y-4">
											<div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
												...
												<div className="space-y-4">
													<Button onClick={exportAwardsCSV} className="w-full bg-blue-600 hover:bg-blue-700">
														<Download className="w-4 h-4 mr-2" />
														Export Awards CSV
													</Button>
													<Button onClick={exportSignatoriesCSV} className="w-full bg-blue-600 hover:bg-blue-700">
														<Download className="w-4 h-4 mr-2" />
											...
											{/* Tips Card */}
											<div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
												<h3 className="text-sm font-medium text-blue-800 mb-2">Import/Export Tips</h3>
												<ul className="text-sm text-blue-700 space-y-1">
				...
				<div className="w-full 2xl:flex-1 relative">
					<div className="sticky top-4">
						<div className="flex items-center gap-2 mb-6">
							...
							<h2 className="text-3xl font-bold text-gray-800">Certificate Preview</h2>
						</div>

						{/* Scrollable container for certificates */}
						<div className="overflow-y-auto 2xl:max-h-[calc(100vh-120px)] pr-4 -mr-4">
							<div className={`space-y-8 ${certificateStyle === 'wide' ? 'max-w-[1000px]' : 'max-w-3xl'}`}>
								{awards.map((award, index) => (
									<div
										key={index}
									...
									>
										...
										<div className="certificate-content relative z-10 h-full flex flex-col items-center justify-center">
											...
											<div
												...
												style={{
													minHeight: certificateStyle === 'wide' ? '500px' : 'auto'
												}}
											>
												{/* Enhanced Decorative Header */}
												<div className="text-center mb-4">
													<div className={`flex items-center justify-center mb-2 ${certificateStyle === 'wide' ? 'mx-16' : 'mx-8'}`}>
                      ...
                      ${decorativeElementsColor}, transparent)`,
														...
														/>
														<h1
															className={`font-bold font-serif tracking-wider ${certificateStyle === 'wide' ? 'text-5xl' : 'text-4xl'}`}
															style={{
													...
													<h2
														className={`font-semibold font-serif mt-2 ${certificateStyle === 'wide' ? 'text-3xl' : 'text-2xl'}`}
														style={{
															color: titleColor
														}}
													>
												...
												<div className="text-center mb-4">
													<p
														className={`italic ${certificateStyle === 'wide' ? 'text-xl mt-14' : 'text-lg mt-8'}`}
														style={{
													...
													<div className="relative inline-block mt-2">
														<p
															className={`font-bold font-serif relative z-10 ${certificateStyle === 'wide' ? 'text-4xl mt-10' : 'text-3xl'}`}
															style={{
													...
													<div className={`flex items-center justify-center my-4 ${certificateStyle === 'wide' ? 'mx-32' : 'mx-16'}`}>
														<Star
															className="w-4 h-4"
															style={{
																color: decorativeElementsColor
												...
												<div className="text-center mb-6">
													<p
														className={`italic leading-relaxed ${certificateStyle === 'wide' ? 'text-xl' : 'text-base'}`}
														style={{
															color: descriptionColor
														}}
													>
												...
												<div className={`flex justify-between ${certificateStyle === 'wide' ? 'mx-24 mt-12 mb-4' : 'mx-12 mt-8 mb-2'}`}>
													{signatories.map((signatory, index) => (
														<div key={index} className="text-center">
															<p
																className={`font-semibold ${certificateStyle === 'wide' ? 'text-lg' : 'text-base'}`}
																style={{
															...
															<div
																className={`w-32 border-t-2 my-1 ${certificateStyle === 'wide' ? 'mx-16' : 'mx-8'}`}
																style={{
																	borderColor: decorativeElementsColor
																}}
															/>
															<p
																className={certificateStyle === 'wide' ? 'text-base' : 'text-sm'}
																style={{
																	color: signatoryColor
																}}
															>
